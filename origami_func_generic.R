origami_func_generic <- function(df, axistype=0, seg=4, pty=16, pcol=1:8, plty=1:6, plwd=1,
  ...
  df <- data.frame(df, df*0+min_value)[order(rep(names(df),2))]
  aux_array <- seq(2,2*n_prime,2)
  colnames(df)[aux_array]=""
  ...
  for (ind in 1:n_col){
    if (ind == 1){
      ...
      if ((ind %% 2) == 0){
        draw.radial.line(0, weights[ind-1], center=c(0,0), deg = 90+(360/n_col)*(ind-1), lwd = 2, lty = 2, col = rgb(0,0,0,0.2))
      } else{
        draw.radial.line(0, weights[ind-1], center=c(0,0), deg = 90+(360/n_col)*(ind-1), lwd = 2, lty = 1, col = rgb(0,0,0,0.2))
      }
