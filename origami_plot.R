...
##### Origami Plot Weighted #####
origami_plot_weighted<- function(df, weight, pcol, pfcol, pcol2 = rgb(0.6,0.3,0.3,1), pfcol2 = NULL, axistype=0, seg=4, pty=16, plty=1:6, plwd=1,
                                 pdensity=NULL, pangle=45, cglty=1.4, cglwd=0.1,
                                 cglcol="#000000", axislabcol="#808080", title="", maxmin=TRUE,
                                 na.itp=TRUE, centerzero=FALSE, vlabels=NULL, vlcex=1,
                                 caxislabels=seq(0,1,by = 0.25), calcex=NULL,
  ...
  aux_array_odd <- as.vector(seq(1,2*n_prime-1,2))
  aux_array_even <- as.vector(seq(2,2*n_prime,2))
  df2_original <- df1[3,aux_array_odd]
