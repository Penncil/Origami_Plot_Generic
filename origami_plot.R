...
origami_plot<- function(df, col, axistype=0, seg=4, pty=16, plty=1:6, plwd=1,
                         pdensity=NULL, pangle=45, cglty=1.4, cglwd=0.1,
                         cglcol="#000000", axislabcol="#808080", title="", maxmin=TRUE,
  ...
  for (ind in 1:n_col){
    if (ind == 1){
      ...
      if ((ind %% 2) == 0){
        draw.radial.line(0, weights[ind-1], center=c(0,0), deg = 90+(360/n_col)*(ind-1), lwd = 1, lty = 2, col = rgb(0,0,0,0.4))
      } else{
        draw.radial.line(0, weights[ind-1], center=c(0,0), deg = 90+(360/n_col)*(ind-1), lwd = 1, lty = 1, col = rgb(0,0,0,0.4))
      }
    }
  }
    ...
    green <- col[2]
    blue <- col[3]
    pcol <- rgb(red,green,blue,1)
    pfcol<- rgb(red,green,blue,0.1)
  #if (centerzero) {
  ...
  if (!is.null(full_point)){
    ...
    if (is.null(dim(full_point))){
      ...
      points(xx[-aux_array_even]*scale[-aux_array_even], 
             yy[-aux_array_even]*scale[-aux_array_even], pch=point_pch, col=point_col)
    }else{
...
origami_plot_pairwise<- function(df1, df2, col1, col2, axistype=0, seg=4, pty=16, plty=1:6, plwd=1,
                        pdensity=NULL, pangle=45, cglty=1.4, cglwd=0.1,
                        cglcol="#000000", axislabcol="#808080", title="", maxmin=TRUE,
                        na.itp=TRUE, centerzero=FALSE, vlabels=NULL, vlcex=1,
                        caxislabels=seq(0,1,by = 0.25), calcex=NULL,
...
area_calculation <- function(df){
  ...
  names <- colnames(df[aux_array_odd])
  sin_angle <- sin(pi/n)
  min_value <- df[3,2]
  for (k in 1:n){
    value <- df[3,2*k-1]
    area <- value*min_value*sin_angle
    result <- cbind(result, area)
  }
